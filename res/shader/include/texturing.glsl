...
#ifdef HIGHQ
  return
    blend.x * texture(s, pos.yz) + 
    blend.y * texture(s, pos.zx) +
    blend.z * texture(s, pos.xy);
#else
...
vec3 sampleTriplanarBumpmap(sampler2D s, vec3 pos) {
  vec3 n = normalize(vertNormal);
  vec3 blend = n * n;
  vec3 tx = 2.0 * texture(s, pos.yz).xyz - 1.0;
  vec3 ty = 2.0 * texture(s, pos.zx).xyz - 1.0;
  vec3 tz = 2.0 * texture(s, pos.xy).xyz - 1.0;
