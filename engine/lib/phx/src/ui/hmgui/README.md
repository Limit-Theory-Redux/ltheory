...
This feature is used for automatic property value transferring from one element to another. Let's take a `button` element as an example. Inside, it contains `text` element that expects `text.font` and `text.color` properties. They cannot be set directly for the button, so we register `button.text-font` and `button.text-color`, and map them to the corresponding `text` properties. Imagine we have defined `button` element in Lua:
```lua
...
- `Gui:mapProperty(id)`: copies property value to it's mapped properties for the current following element. Should be used inside element function definition.
...
1. Default properties. Those are hardcoded in the engine in [core_properties.rs](core_properties.rs) file. These properties are used if property was not found in other 2 groups. There is an autogenerated enum containing ids of all core properties: `GuiProperties`.
2. Themes. Theme is a big set of properties that overhauls the default ones. Theme should be defined in a separate Yaml file in the `theme` resource folder. Name of the file will be used as the theme name. All themes are automatically loaded by the engine during startup.
Methods to work with themes in the scripts:
...
Theme property value is used if it was not found in the last group.
3. Element styles. This type of style is used for per element styling. There are 2 ways to do it:
  - explicitly in the code. Example:
```lua
Gui:clearStyle()
Gui:setPropertyVec4(GuiProperties.ButtonTextColorId, Vec4(1, 0, 0, 1))
Gui:button("MyButton")
```
  - via style configuration file. During startup the engine loads all element styles from the `styles.yaml` configuration file in the resource folder. [Example](../../../test_data/styles.yaml). Usage:
```lua
Gui:setStyle(Enums.Gui.Styles.MyButtonStyleId)
Gui:button("MyButton")
```
Ids of the element styles should be initialized in the [`GuiEnums.lua`](/script/Enums/GuiEnums.lua) file.
...
Scripters can define a custom properties in the [`GuiEnums.lua`](/script/Enums/GuiEnums.lua) file. Example:
```lua
