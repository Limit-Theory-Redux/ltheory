...
typedef struct ShaderVar ShaderVar;
struct ShaderVar {
  ShaderVarType type;
  cstr name;
  int index;
...
static int GetUniformIndex (Shader* self, cstr name) {
  if (!self)
...
// static int GetUniformIndexMustSucceed (Shader* self, cstr name) {
//   if (!self)
//     Fatal("GetUniformIndex: No shader is bound");
//   int index = glGetUniformLocation(self->program, name);
//   if (index == -1)
...
void Shader_Stop (Shader* s) {
  GLCALL(glUseProgram(0))
  current = 0;
}

static void ShaderCache_FreeElem (cstr s, void* data) {
  free(data);
}
