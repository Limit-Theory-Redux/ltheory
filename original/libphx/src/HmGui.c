...
#define Widget_Rect 2
#define Widget_Image 3

#define FocusStyle_None 0
#define FocusStyle_Fill 1
#define FocusStyle_Outline 2
#define FocusStyle_Underline 3

#define FocusType_Mouse 0
#define FocusType_Scroll 1
#define FocusType_SIZE 2

typedef struct HmGuiWidget HmGuiWidget;
struct HmGuiWidget {
...
typedef struct HmGuiGroup HmGuiGroup;
struct HmGuiGroup {
  HmGuiWidget widget;
  HmGuiWidget* head;
  HmGuiWidget* tail;
...
typedef struct HmGuiText HmGuiText;
struct HmGuiText {
  HmGuiWidget widget;
  Font* font;
  cstr text;
...
typedef struct HmGuiRect HmGuiRect;
struct HmGuiRect {
  HmGuiWidget widget;
  Vec4f color;
};

typedef struct HmGuiImage HmGuiImage;
struct HmGuiImage {
  HmGuiWidget widget;
  Tex2D* image;
};

typedef struct HmGuiClipRect HmGuiClipRect;
...
typedef struct HmGuiData HmGuiData;
struct HmGuiData {
  Vec2f offset;
  Vec2f minSize;
  Vec2f size;
...
typedef struct HmGuiStyle HmGuiStyle;
struct HmGuiStyle {
  HmGuiStyle* prev;
  Font* font;
  float spacing;
...
void HmGui_Begin (float sx, float sy) {
  ...
  self.group->widget.pos = Vec2f_Create(0, 0);
  self.group->widget.size = Vec2f_Create(sx, sy);
...
void HmGui_Image (Tex2D* image) {
  ...
  HmGui_InitWidget(&e->widget, Widget_Image);
  e->image = image;
...
void HmGui_Rect (float sx, float sy, float r, float g, float b, float a) {
  ...
  HmGui_InitWidget(&e->widget, Widget_Rect);
...
void HmGui_TextEx (Font* font, cstr text, float r, float g, float b, float a) {
  ...
  HmGui_InitWidget(&e->widget, Widget_Text);
  e->font = font;
  e->text = StrDup(text);
  e->color = Vec4f_Create(r, g, b, a);
  Vec2i size; Font_GetSize2(e->font, &size, e->text);
  e->widget.minSize = Vec2f_Create(size.x, size.y);
...
bool HmGui_GroupHasFocus (int type) {
  ...
  return self.focus[type] == self.group->widget.hash;
}
