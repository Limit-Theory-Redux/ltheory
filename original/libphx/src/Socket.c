...
void Socket_Bind (Socket* self, int port) {
  ...
  if (bind(self->sock, (struct sockaddr_in*)&addr, sizeof(addr)) == SOCK_ERROR)
...
int Socket_ReceiveFrom (Socket* self, void* data, size_t len) {
  ...
  socklen_t addrSize;
#if POSIX
  int bytes = recvfrom(self->sock, data, len, 0, (struct sockaddr_in*)&self->addrRecv, &addrSize);
#else
  int bytes = recvfrom(self->sock, (char*)data, (int)len, 0, (struct sockaddr_in*)&self->addrRecv, &addrSize);
#endif
...
#if POSIX
  int bytes = sendto(self->sock, data, len, 0, (struct sockaddr_in*)&self->addrSend, sizeof(self->addrSend));
#else
  int bytes = sendto(self->sock, (cstr)data, (int)len, 0, (struct sockaddr_in*)&self->addrSend, sizeof(self->addrSend));
#endif
  if (bytes == SOCK_ERROR)
    return -1;
  return bytes;
