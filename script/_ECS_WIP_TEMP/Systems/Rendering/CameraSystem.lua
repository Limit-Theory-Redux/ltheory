...
function CameraSystem:onRender()
    ...
    if self.cameras then
        ---@param camera Camera
        for camera in Iterator(self.cameras) do
            local cdt = camera:findComponentByArchetype(Enums.ComponentArchetype.CameraDataComponent)
            ---@cast cdt CameraDataComponent

            self:beginCameraDraw(cdt)
        end
    end
    self.profiler:stop()
end
