...
function RuleEvaluator.evaluate(rng, rule, context)
    ...
    elseif rule.type == Enums.Gen.Rule.ByType then
        if not rule.types or type(rule.types) ~= "table" then
            Log.Error("ByType rule requires a 'types' table")
            return false
        end
        for typeKey, values in pairs(rule.types) do
            if type(values) ~= "table" or #values == 0 then
                Log.Error("ByType rule '%s' has invalid values for type '%s'", tostring(rule.name), tostring(typeKey))
                return false
            end
            for _, v in ipairs(values) do
                if not v.weight or not v.id then
                    Log.Error("ByType rule value missing 'id' or 'weight' in type '%s'", tostring(typeKey))
                    return false
                end
            end
        end
...
---@param overrideWeights table<any, number>|nil
---@return any
function RuleEvaluator.evaluateWeighted(rng, values, overrideWeights)
    ...
    local totalWeight = 0
    for _, v in ipairs(values) do
        -- Handle both table values (with .type) and direct enum values
        local key = type(v.value) == "table" and v.value.type or v.value
        local weight = overrideWeights and overrideWeights[key] or v.weight
        totalWeight = totalWeight + weight
    end

    if totalWeight == 0 then
        Log.Error("Weighted rule has zero total weight, using first value")
    ...
    Log.Warn("Weighted rule fallback to first value (shouldn't happen)")
    return values[1].value
end
