...
local conditionHandlers = {
    OrbitRadius = function(rng, rule, context)
    ...
    PlanetType = function(rng, rule, context)
        ...
        if typeRules then
            ...
            elseif rule.type == "Range" then
                return rng:getUniformRange(typeRules.min, typeRules.max)
            end
        end
        return nil
    end,
    PlanetSize = function(rng, rule, context)
    ...
    SystemAge = function(rng, rule, context)
        ...
        for _, range in ipairs(rule.condition.ranges) do
            if systemAge >= range.min and systemAge <= range.max then
                if rule.type == "Weighted" then
                    return RuleEvaluator.evaluateWeighted(rng, rule.values, range.weights)
                elseif rule.type == "Fixed" then
                    return range.value
                end
            end
        end
    ...
    StarType = function(rng, rule, context)
    ...
    StarMass = function(rng, rule, context)
    ...
    SystemMetallicity = function(rng, rule, context)
    ...
    PlanetCount = function(rng, rule, context)
...
function RuleEvaluator.evaluate(rng, rule, context)
    ...
    if rule.condition then
        ...
        if handler then
            ...
            Log.Debug("Condition %s not met for rule type: %s", rule.condition.type, rule.type)
            return rule.default or (rule.type == "Weighted" and rule.values[1].value) or rule.value or 0
        end
        Log.Error("Unknown condition type: %s", rule.condition.type)
        return rule.default or 0
    end

    if rule.type == "Count" then
