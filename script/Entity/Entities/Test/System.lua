...
local Dust = require('Entity.Entities.Effects.Dust')
local Nebula = require('Entity.Entities.Objects.Nebula')
local ShipType = require('Entity.Entities.Ship.ShipType')
...
function System:spawnAsteroidField (count, oreCount)
  ...
  for i = 1, count do
    ...
    local asteroid = Entities.Objects.Asteroid(rng:get31(), scale)
    asteroid:setPos(pos)
...
function System:spawnPlanet ()
  local rng = self.rng
  local planet = Entities.Objects.Planet(rng:get64())
...
function System:spawnShip ()
  ...
  if true then
    while true do
      local thruster = Entities.Ship.Thruster()
  ...
  if true then
    while true do
      local turret = Entities.Ship.Turret()
...
function System:spawnStation ()
  local station = Entities.Objects.Station(self.rng:get31())
