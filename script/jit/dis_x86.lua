...
local function putop(ctx, text, operands)
    ...
    if ctx.o16 then
        text = "o16 " .. text; ctx.o16 = false
    end
    if ctx.a32 then
        text = "a32 " .. text; ctx.a32 = false
    end
    if ctx.rep then
        text = ctx.rep .. " " .. text; ctx.rep = false
    end
    if ctx.rex then
        ...
        elseif p == "1" then
            ...
            if not mode then
                ...
                if rm and ctx.rexb then
                    rm = rm + 8; ctx.rexb = false
                end
                if ctx.rexr then
                    sp = sp + 8; ctx.rexr = false
                end
            end
            if p == "m" then
                if mode == 3 then
