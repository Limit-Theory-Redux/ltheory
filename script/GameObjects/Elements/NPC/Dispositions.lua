...
function Entity:setDisposition(target, value)
    if self ~= Config.game.currentShip then
        ...
        if oldDispo ~= value then
            ...
            --[[
            Log.Debug("Disposition of %s to %s was %f, is now %f", self:getName(), target:getName(), oldDispo,
                self.dispositions[target])
            ...
            if dispoNameIndex ~= 0 then
                ...
                Log.Debug("%s is now %s to %s%s", self:getName(), Config.game.dispoName[dispoNameIndex], target:getName(),
                    surprise)
            end
            ]]--
        end
