...
bool ImGui_ButtonEx (cstr label, float sx, float sy) {
  ...
  EmitPanel(color, self.widget->pos, self.widget->size, focus ? 1.0f : 0.5f, 4.0f);
...
bool ImGui_Checkbox (bool value) {
  ...
  EmitPanel(
    ...
    Vec2f_Create(self.widget->size.x - 4, self.widget->size.y - 4), 1.0f, 4.0f);
