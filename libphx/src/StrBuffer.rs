    ...
    if (newSize > (*this).capacity) as libc::c_long != 0 {
        while (*this).capacity < newSize {
            (*this)
                ...
                .wrapping_mul(2 as libc::c_int as libc::c_uint) as u32 as u32;
        }
        (*this)
            .data = MemRealloc(
...
unsafe extern "C" fn StrBuffer_AppendData(
    ...
    (*this)
        .size = ((*this).size as libc::c_uint).wrapping_add(len) as u32 as u32;
}
#[no_mangle]
...
unsafe extern "C" fn StrBuffer_SetImpl(
    ...
    if (newSize as u32 <= (*this).capacity) as libc::c_long != 0 {
        ...
        return (newSize as libc::c_uint).wrapping_sub((*this).capacity) as i32
    };
}
#[no_mangle]
