...
pub unsafe extern "C" fn HashGrid_Create(
    ...
    (*self_0)
        .cells = MemAllocZero(
        (::core::mem::size_of::<HashGridCell>() as libc::c_ulong)
    ...
    (*self_0)
        ...
        (0x1000 as libc::c_uint as libc::c_ulong)
            .wrapping_div(::core::mem::size_of::<HashGridElem>() as libc::c_ulong)
            as uint32,
    );
...
pub unsafe extern "C" fn HashGrid_QueryPoint(
    ...
    while i < (*cell).elems_size {
            ...
            as libc::c_long != 0
        {
            ...
            *pData = MemRealloc(
                (*self_0).results_data as *mut libc::c_void,
                ((*self_0).results_capacity as libc::c_ulong).wrapping_mul(elemSize),
            );
        }
